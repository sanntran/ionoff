...
const char http_404_error[] = "404:Not found";
const char http_500_error[] = "500:Internal Error";
...
int1 tcp_http_put_file(int8 which, int16 errorCode, int32 file,
		char *contentType) {
	static char str[40];
	int8 socket;
	char ec;
...
void HTTP_Task(void) {
	...
	static enum {
		HTTP_REQ_GET = 1, HTTP_REQ_POST = 2, HTTP_REQ_UNKOWN = 0
	} http_cmd[HTTP_NUM_SOCKETS] = { 0 };
	...
	for (hs = 0; hs < HTTP_NUM_SOCKETS; hs++) {
		...
		case HTTP_GET_HEADERS:
				...
				if (c == '\n') {
					...
					if (((pKey = strtok(&buffer[hs][0], tokens_header)) != 0)
						...
						if ((strcmp(pKey, http_get_str) == 0)
							...
							if (pValue)
								http_parse_cgi_string(http_page_req[hs],
										pValue);
			...
			if (TickGetDiff(TickGet(), http_timer[hs])
					> TICKS_PER_SECOND * 20) {
		...
		case HTTP_SEND_RESPONSE_CONTINUE:
			...
			if (TCPIsPutReady(currSocket)) {
				...
				if (http_page_req[hs] == 0xFFFFFFFF)
					doneSend = tcp_http_put_file(hs, 500,
							label_address(http_500_error), 0);
					...
					doneSend = tcp_http_put_file(hs, 404,
							label_address(http_404_error), 0);
		...
		case HTTP_CLOSE_WAITING:
			TCPDiscard(currSocket);
			if (TCPIsPutReady(currSocket)
					|| (TickGetDiff(TickGet(), http_timer[hs])
							> (TICKS_PER_SECOND * 5))) {
